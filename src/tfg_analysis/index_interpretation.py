"""Interpretacion tactica estable de IDD, IPO y xThreat."""
...
INDEX_LEVELS = {
    IDD_METRIC: [
        {"label": "Estable", "description": "La estructura mantiene orden relativo.", "color": "#38c172"},
        {"label": "Vulnerable", "description": "Aparecen desajustes, todavia sin ruptura clara.", "color": "#f4c542"},
        {"label": "Inestable", "description": "Hay perdida relevante de control defensivo.", "color": "#ff9f43"},
        {"label": "Critica", "description": "La estructura queda muy expuesta.", "color": "#e4143a"},
    ],
    IPO_METRIC: [
        {"label": "Baja", "description": "La secuencia genera poco peligro potencial.", "color": "#38c172"},
        {"label": "Moderada", "description": "El rival progresa, pero sin alcanzar peligro alto.", "color": "#f4c542"},
        {"label": "Alta", "description": "La accion alcanza condiciones ofensivas relevantes.", "color": "#ff9f43"},
        {"label": "Critica", "description": "La secuencia combina ventaja ofensiva y alto peligro.", "color": "#e4143a"},
    ],
}

IPO_MANUAL_THRESHOLDS = {"p25": 0.15, "p50": 0.35, "p75": 0.60, "method": "manual"}
...
def _load_sequence_population(app_data_dir: str | Path) -> pd.DataFrame:
...
def _coerce_sequence_population(source: pd.DataFrame | str | Path | None) -> pd.DataFrame:
    ...
    if source is None:
        return pd.DataFrame()
    return _load_sequence_population(source)
...
def classify_index_value(
    ...
    if value is None or pd.isna(value):
        return {"label": "-", "level": 0, "color": "#6b7280", "description": "Sin dato disponible.", "value": None}

    reference = thresholds.as_dict() if isinstance(thresholds, ThresholdReference) else thresholds
    val = float(value)
...
def build_interpretability_reference(source: pd.DataFrame | str | Path | None) -> dict[str, dict[str, object]]:
    sequences = _coerce_sequence_population(source)
    ...
    for metric in (IDD_METRIC, IPO_METRIC):
        ...
        for raw_value in _clean_values(values):
            ...
            if 1 <= level <= 4:
                counts[level - 1] += 1
        references[metric] = {"thresholds": thresholds.as_dict(), "counts": counts, "levels": INDEX_LEVELS[metric]}
    return references
...
def xthreat_reference_text() -> list[dict[str, str]]:
    return [
        ...
        {"label": "Moderada", "range": "0.05 - 0.10", "color": "#f4c542", "description": "Progresion con amenaza reconocible."},
        {"label": "Alta", "range": "0.10 - 0.20", "color": "#ff9f43", "description": "Entrada en zonas de peligro relevante."},
        {"label": "Muy peligrosa", "range": ">= 0.20", "color": "#e4143a", "description": "Cercania al area y alto potencial de ocasion."},
    ]
