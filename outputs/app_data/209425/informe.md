# Informe del partido 209425

El analisis detecta **45 secuencias ofensivas rivales**. El rival registra **10 tiros oficiales**, de los cuales **6** van a puerta. Dentro de las secuencias analizadas quedan asociados **6 tiros**.

## Lectura defensiva global

El **IDD medio** del partido es **0.40** y el **IPO medio** es **0.35**. La causa defensiva dominante es **estructura_anchura** y la zona/carril que concentra mas dano eficiente es **carril_central**.

## Patron prioritario

El cluster de mayor prioridad analitica es el **cluster 0.0**, con **15.0 secuencias**, IDD medio **0.47** e IPO medio **0.45**.

## Interpretacion

El foco del informe no es explicar todos los tiros aislados del rival, sino los tiros y acciones peligrosas que aparecen dentro de las secuencias ofensivas detectadas por la metodologia. Esto permite estudiar patrones comparables de desorganizacion defensiva y relacionarlos con la peligrosidad real de la accion.
